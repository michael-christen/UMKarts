...
static void _xbee_interface_rx_handler(mss_uart_instance_t * this_uart) {
  ...
  do {
    ...
    bytes_from_uart = MSS_UART_get_rx(this_uart, _xbee_rx.buf + bytes_remaining, MSS_UART_RX_BUF_SIZE - bytes_remaining);

    /* Parse packets from uart into our packet */
    end_of_read = XBeeReaderRead(&(_xbee_rx.xr), _xbee_rx.buf, _xbee_rx.buf + bytes_from_uart + bytes_remaining);
    ...
    if (XBeeReaderDone(&(_xbee_rx.xr))) {
      ...
      bytes_remaining = bytes_from_uart - (end_of_read - _xbee_rx.buf);
