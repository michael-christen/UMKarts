 ...
 *
 *   Description  : Device driver for the on-board OLED (white OLED Module) on
...
#define CMD_VER_DISPLAY_OFFSET 0xD3
#define VER_OFFSET_VALUE 0x00
/***************************************************************************//**
    ...
    switch(LINES)
      {
          ...
          case THIRD_LINE:
          {
              start_line = THIRD_LINE;
              end_line = THIRD_LINE;
          }
          case FOURTH_LINE:
          {
              start_line = FOURTH_LINE;
              end_line = FOURTH_LINE;
          }
          case FIFTH_LINE:
          {
              start_line = FIFTH_LINE;
              end_line = FIFTH_LINE;
          }

          case FIRST_TWO_LINES:
          {
              start_line = FIRST_LINE;
              end_line = SECOND_LINE;
          }
...
void OLED_vertical_scroll(struct oled_data * ver_scroll)
{
    ...
    uint8_t ver_scroll_setup_data[] =
    {
    		OLED_COMMAND_CODE, CMD_VER_DISPLAY_OFFSET,
    		OLED_COMMAND_CODE, VER_OFFSET_VALUE
    };

    if(ver_scroll->vertical_on_off == 1)
    {
    	ver_scroll_setup_data[3] = ver_scroll->vert_scroll_offset;
        MSS_I2C_write( g_p_oled_i2c, OLED_SLAVE_ADDRESS, ver_scroll_setup_data, sizeof(ver_scroll_setup_data), MSS_I2C_RELEASE_BUS );
        MSS_I2C_wait_complete( g_p_oled_i2c );
    }
}
/***************************************************************************//**
...
void OLED_cont_vert_and_horiz_scroll(struct oled_data * horiz_scroll)
{
    ...
    uint8_t horiz_scroll_setup_data[] =
    {
        ...
        OLED_COMMAND_CODE, DUMMY_BYTE_00H,
        OLED_COMMAND_CODE, SCROLL_PAGE_0,
        OLED_COMMAND_CODE, SCROLL_256_FRAMES,
        OLED_COMMAND_CODE, SCROLL_PAGE_4,
        OLED_COMMAND_CODE, DUMMY_BYTE_00H,
    };
...
/***************************************************************************//**
 * put_pixel
   To turn on any pixel.
 */

void put_pixel(float value)
{
  static unsigned char x1=0;
  unsigned char line;
  unsigned int plot_value;
  unsigned char plot_map = 0x80;
  uint8_t oled_pixel[] =
  {
      OLED_DATA_CODE, 0x00,
  };
  ...
  plot_value =(unsigned char)(value/SCALE_FACTOR);
  OLED_set_cursor(line,x1);


  oled_pixel[1] = plot_map >> plot_value;
  MSS_I2C_write( g_p_oled_i2c, OLED_SLAVE_ADDRESS, oled_pixel, sizeof(oled_pixel), MSS_I2C_RELEASE_BUS );
  MSS_I2C_wait_complete( g_p_oled_i2c );
    ...
    switch(LINES)
       {
           ...
           case THIRD_LINE:
           {
               OLED_clear_display(THIRD_LINE);
               line = data->line3;
               char_offset = data->char_offset3;
               string = data->string3;
           }
           case FOURTH_LINE:
           {
               OLED_clear_display(FOURTH_LINE);
               line = data->line4;
               char_offset = data->char_offset4;
               string = data->string4;
           }
           case FIFTH_LINE:
           {
               OLED_clear_display(FIFTH_LINE);
               line = data->line5;
               char_offset = data->char_offset5;
               string = data->string5;
           }
           case FIRST_TWO_LINES:
           {
               OLED_clear_display(FIRST_TWO_LINES);
               line = data->line1;
               char_offset = data->char_offset1;
               string = data->string1;
